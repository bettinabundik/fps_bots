...
namespace FPSbots
{
    public class Bot
    {
        ...
        public void Execute(List<List<FieldType>> map, Double gamma, Configuration config, List<Bot> others, Double allactions_max_q_value,
            out Action nextaction, out Double q_value, out Double etvalue, out Double overallreward,
            ...
            out List<Position> items_pickedup)
        {
            ...
            if (init)
            {
                for (int i = 0; i < botnum; i++)
                {
                    ...
                    // Update statistics
                    stat.UpdateStats(bots[i].Id, collision, distancetravelled, pickupitem, iteration, nextpos, nextaction, bots[i].TimeStamp);
                }
            }
                ...
                for (int i = 0; i < botnum; i++)
                {
                    if (bots[i].Alive())
                    {
                        ...
                        bots[i].Execute(map, table_nav.Gamma, config, bots, allactions_max_q_value,
                            out nextaction, out new_q_value, out new_etvalue, out overallreward,
        ...
        public void Execute_Combat(List<List<FieldType>> map, Configuration config, Boolean init, Statistics stat, Int32 iteration)
        {
            ...
            if (init)
            {
                for (int i = 0; i < botnum; i++)
                {
                    ...
                    // Update statistics
                    stat.UpdateStats(bots[i].Id, collision, distancetravelled, false, iteration, nextpos, nextaction, bots[i].TimeStamp);
